...
//	close_fd();
    ...
    if (setsockopt( 3, SOL_SOCKET, SO_SNDTIMEO,&sendtimeout, sizeof(sendtimeout)) == -1)
        printf("setsockopt error\n");
#if 0
    int sendbufint;
    sendbufint=268435456;
    if (setsockopt( 3, SOL_SOCKET, SO_SNDBUF,&sendbufint, sizeof( sendbufint)) == -1)
        printf("setsockopt error\n");
#endif
#if 0
	if(argc == 4){
    ...
    if(!sendbuf){
        printf("malloc error, MAXBUF=%d\n",MAXBUF);
        return -1;
    }
    int nwrite;
    size_t send_size;
    while(tol_size < size){
        send_size = ((size - tol_size) < MAXBUF)?  (size - tol_size) : MAXBUF;
        if((nwrite = read(4,sendbuf,send_size)) > 0){
            if(writen(3,sendbuf,nwrite) != nwrite){
                printf("error\n");
                goto end;
            }
        }else{
